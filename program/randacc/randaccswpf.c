...
#ifdef STRIDE
      __builtin_prefetch(&ran[j+FETCHDIST]);
#endif
    
      ran[j] = (ran[j] << 1) ^ ((s64Int) ran[j] < 0 ? POLY : 0);
      Table[ran[j] & (TableSize-1)] ^= ran[j];
      
      u64Int temp = (ran[(j+(FETCHDIST>>1))%128] << 1) ^ ((s64Int) ran[(j+(FETCHDIST>>1))%128] < 0 ? POLY : 0);
      __builtin_prefetch(&Table[temp & (TableSize-1)]);
      //Add the above instructions in for a manual software prefetch.
    }
  }
}
