...
#ifdef STRIDE
__builtin_prefetch(&(vlist[k+12]));
#endif
    if(k+8 < k2) {
__builtin_prefetch(&XOFF(vlist[k+8]));      	
__builtin_prefetch(&xadj[XOFF(vlist[k+4])]);
      	__builtin_prefetch(&xadj[XOFF(vlist[k+4])+8]);
__builtin_prefetch(&bfs_tree[xadj[XOFF(vlist[k+1])]]);      
__builtin_prefetch(&bfs_tree[xadj[XOFF(vlist[k+1])+1]]);
__builtin_prefetch(&bfs_tree[xadj[XOFF(vlist[k+1])+2]]);
__builtin_prefetch(&bfs_tree[xadj[XOFF(vlist[k+1])+3]]);
__builtin_prefetch(&bfs_tree[xadj[XOFF(vlist[k+1])+4]]);
__builtin_prefetch(&bfs_tree[xadj[XOFF(vlist[k+1])+5]]);
__builtin_prefetch(&bfs_tree[xadj[XOFF(vlist[k+1])+6]]);
__builtin_prefetch(&bfs_tree[xadj[XOFF(vlist[k+1])+7]]);
...
#ifdef STRIDE
	__builtin_prefetch(&xadj[vo+32]);
#endif
	__builtin_prefetch(&bfs_tree[xadj[vo+16]]);
	if (bfs_tree[j] == -1) {
	  bfs_tree[j] = v;
